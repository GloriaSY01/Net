...
const double max_waitingtime = 0.01 * CLOCKS_PER_SEC;
u_long non_block = 1;
u_long block = 0;
...
void receive_MSG()
{
    ...
    header e_header;
    int header_size = sizeof(e_header);
    char* recbuff = new char[header_size + maxsize_data];//接收的包包含了数据
    char* sendbuff = new char[header_size];//发送的包只需要包含数据头
    int hope_seq = 0;
    int send_ack = 0;
    int offset = 0;//message里的偏移
    ioctlsocket(s, FIONBIO, &block);//阻塞接收
    while (true)
    {
        //等待接收数据包
        ...
        if (e_header.seq == hope_seq && checksum((u_short*)recbuff, header_size + e_header.length)==0)
        {
            ...
            cout << "[计算校验和]: " << 0 << endl;
            cout << "[数据接收]:   收到了正确的 [" << e_header.seq << "]号数据包 " << endl;
            ...
            e_header.checksum = 0;
            e_header.checksum = checksum((u_short*)&e_header, header_size);
            memcpy(recbuff, &e_header, header_size);
            //router原因，客户端一定会收到
            sendto(s, recbuff, header_size, 0, (sockaddr*)&router_addr, routerlen);
            //更新hope_seq和ack
            hope_seq = (hope_seq + 1) % 2;//下一次期待的seq
            send_ack = hope_seq;//下一次要发送的ack
